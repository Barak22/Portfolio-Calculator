...
class PortfoliosAnalyzer(portfoliosReader: PortfoliosReader, portfoliosWriter: DataWriter) {
  def analyzePortfolios(stocksNames: Seq[String], fromFile: String, toFile: String) = {
    val minimumPortfolios = buildMapOf(portfoliosReader.readVectorsResultFile(stocksNames, fromFile))
